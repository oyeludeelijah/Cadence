...
function CreateTask({ onTaskCreated, onIsDirtyChange, onUpgradeRequired }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ── Steps 2+3: Atomically insert task + checkpoints via RPC ─────────────
      // A single Postgres transaction — if checkpoints fail, the task is never
      // created. No client-side rollback needed.
      const isAI = checkpointSource === 'ai'
      const checkpointRows = rawSequence.map((cp) => ({
      ...
      const { error: rpcError } = await supabase.rpc('create_task_with_checkpoints', {
        p_task: {
      ...
      const countLabel = `${checkpointRows.length} checkpoint${checkpointRows.length !== 1 ? 's' : ''}`
